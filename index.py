    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                upper = np.array([50, 250, 255], dtype="uint8")
                kernel_size = 5
                kernel = np.ones((kernel_size, kernel_size),
                                 np.float32)/kernel_size/kernel_size
                kernel2 = np.ones((10, 10), np.uint8)/100
                converted = cv2.cvtColor(croppedImage, cv2.COLOR_BGR2HSV)
                ...
                skin = cv2.bitwise_and(
                    croppedImage, croppedImage, mask=skinMask)
                # show the skin in the image along with the mask
                cv2.imshow("images", np.hstack([croppedImage, skin]))
